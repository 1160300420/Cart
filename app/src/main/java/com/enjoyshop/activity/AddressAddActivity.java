...
public class AddressAddActivity extends BaseActivity {
    ...
    private int temp_sid=1;//用于计算addressid
    /**
     * 默认只有1条数据
     */
    ...
    protected void init() {
        ...
        temp_sid= intent.getIntExtra("addressid",0)+1;
    ...
    public void createAddress() throws InterruptedException {
        ...
        String userId = EnjoyshopApplication.getApplication().getUser().getUsername();

        List<Address> mAddressDataList = new DataManager().queryAddress(userId);
        ...
        addBean.setUserId(Long.parseLong(userId));//userid为手机号
        addBean.setAddressId(Long.parseLong(temp_sid+""));
        ...
        if (addressDoType == 0) {
            if(new DataManager().insertAddress(addBean)==0) {
                ToastUtils.showSafeToast(AddressAddActivity.this, "地址添加成功");
            }
            else{
                ToastUtils.showSafeToast(AddressAddActivity.this, "地址添加失败");
            }
        } else if (addressDoType == 1) {
            Long addressId = (Long) getIntent().getSerializableExtra("addressId");
            addBean.setAddressId(addressId);
            new DataManager().updateAddress(addBean);
            new DataManager().insertAddress(addBean);
